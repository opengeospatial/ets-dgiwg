...
public class TemporalFilterTest extends AbstractTemporalFilterTest {
    ...
    private static final List<String> EXPECTED_TEMPORAL_OPERATORS = Arrays.asList( "After", "Before", "Begins",
                                                                                   "BegunBy", "TContains", "TEquals",
                                                                                   "TOverlaps", "Meets",
                                                                                   "OverlappedBy", "MetBy", "Ends",
                                                                                   "EndedBy" );
    ...
    public void additionalTemporalOperatorIsEnabled()
        ...
        assertTrue( supportedTemporalOperators.size() > 0,
                    "TemporalOperators are not valid. At least one of the temporal operators "
                                    + EXPECTED_TEMPORAL_OPERATORS + " must be supported" );
    }

    @Test(description = "DGIWG - Web Feature Service 2.0 Profile, 7.3.3., S.24, "
                        + "Requirement 10; OGC 09-026R2 Temporal Filter CC, A.10", dataProvider = "protocol-featureType-duringOperator")
    public
                    void withDuringFilter( ProtocolBinding binding,
    ...
    @Test(description = "DGIWG - Web Feature Service 2.0 Profile, 7.3.3., S.24, "
                        + "Requirement 10; OGC 09-026R2 Temporal Filter CC, A.10", dataProvider = "protocol-featureType-temporalOperator")
    public
                    void withTemporalOperatorFilter( ProtocolBinding binding,
                                                     FeatureTypeToPropertyAndValue featureTypeToPropertyAndValue,
